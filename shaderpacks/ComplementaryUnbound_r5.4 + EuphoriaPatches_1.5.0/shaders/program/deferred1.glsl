...
vec2 calculateVelocityOffset() {
    vec4 prevViewDir = vec4(0, 0, -1, 0);
    mat4 invPrevModelView = inverse(gbufferPreviousModelView);
    vec3 worldSpaceDir = (invPrevModelView * prevViewDir).xyz;
    return worldToView(worldSpaceDir).xy;
}

float computeFlashlightIntensity(vec2 screenPos, float sceneDepth) {
    // Apply motion blur offset
    float velocityScale = 0.024 / frameTime;
    vec2 adjustedPos = screenPos - clamp(calculateVelocityOffset(), -1.0, 1.0) * velocityScale;
    
    // Determine if flashlight should be on
    #if (FLASHLIGHT_MODE == 1)
    float isFlashlightActive = step(0.4, night);
    #else
    float isFlashlightActive = 1.0;
    #endif
    
    // Calculate normalized distance from screen center
    vec2 screenDims = vec2(viewWidth, viewHeight);
    vec2 centerOffset = (adjustedPos - vec2(0.5)) * screenDims;
    float normalizedDist = length(centerOffset) / screenDims.x;
    
    // Apply projection correction
    float projectedDist = normalizedDist * 3.8 / gbufferProjection[1][1];
    
    // Calculate falloff pattern
    float falloff = -pow2(projectedDist + 0.2) + 1.0;
    float pattern = cos(12.5 * projectedDist) * 0.35 + 0.75;
    float visibility = max(falloff * pattern, 0.0);
    
    // Combine depth and visibility
    float depthFactor = pow4(sceneDepth) * sceneDepth + 3.0 * pow3(sceneDepth);
    return max(1.0, 2.0 * depthFactor * visibility);
}
...
void main() {
    ...
    if (z0 < 1.0) {
        ...
        // Apply flashlight to final color
        // float flashlightVis;
        // color.rgb *= computeFlashlightIntensity(texCoord, GetLinearDepth(z0) * -1.0 + 1.0);
